...
class EntityIndex:
    ...
    def find_by_alias(self, alias: str) -> Optional[IndexEntry]:
        ...
        Uses FTS5 to narrow candidates, then verifies exact match in Python.
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Use FTS to narrow candidates, then verify exact match
            fts_query = self._escape_fts_query(alias)
            rows = conn.execute(
                """
                SELECT e.* FROM entities e
                JOIN entities_fts fts ON e.rowid = fts.rowid
            ...
            # FTS may miss aliases with special characters; fall back to name check
            row = conn.execute(
    ...
    def find_by_email_domain(self, domain: str) -> List[IndexEntry]:
        ...
        Uses SQL LIKE to narrow candidates (email_domains is JSON array stored as text).
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            # email_domains is stored as JSON array text like '["anthropic.com"]'
            # Use LIKE to narrow, then verify in Python
            rows = conn.execute(
                "SELECT * FROM entities WHERE LOWER(email_domains) LIKE ?",
                (f"%{domain_lower}%",),
            ).fetchall()
