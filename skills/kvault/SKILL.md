...
**Version handshake.** This skill is written for kvault **0.15.1+**. Run `kvault --version`
...
went exactly as asked. The 11 codes:
...
- When the owner corrects you, fix the tree and then record the decision with `kvault mark`
  (`--distinct-from`, `--max-children`, `--series-ok`) so `check`, `plan`, and the create
  guard stop re-proposing it. A correction that lives only in the conversation evaporates.
...
| Quality | `kvault validate`, `kvault check [--summary-max-words N\|0] [--summary-max-dated-sections N\|0] [--max-children N]`, `kvault plan [PATH] [--limit N\|0]`, `kvault mark <path> [--distinct-from X] [--max-children N] [--series-ok] [--clear]` |
