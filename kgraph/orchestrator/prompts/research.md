...
1. **Search the index database**
   ```bash
   sqlite3 {kg_root}/.kgraph/index.db "SELECT path, name, aliases FROM entities WHERE name LIKE '%{name}%' OR aliases LIKE '%{name}%'"
   ```

2. **Search by email domain** (if email provided)
   ```bash
   sqlite3 {kg_root}/.kgraph/index.db "SELECT path, name, aliases FROM entities WHERE aliases LIKE '%{email_domain}%'"
   ```
